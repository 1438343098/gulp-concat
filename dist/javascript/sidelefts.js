...
//		alert(document.documentElement.clientWidth+','+$('.sideright').width())
	...
	function sidehid(i, classnames1, classnames2) {
		$('.slidtoggle' + i).click(function() {
			...
			$(this).parent('.lislide').siblings('.lislide').children('.slidtoggle' + i).children('.imgtoggle').children('.imgright').show()
			$(this).parent('.lislide').siblings('.lislide').children('.slidtoggle' + i).children('.imgtoggle').children('.imgdown').hide()
			$(this).parent('.lislide').siblings('.lislide').children('.' + classnames1).children('.' + classnames2).children('.imgright').hide()
			$(this).parent('.lislide').siblings('.lislide').children('.' + classnames1).children('.' + classnames2).children('.imgdown').show()
		})
	}
	sidehid(0)
	...
	function sideclick(classnames) {
		$('.' + classnames).click(function() {
			$('.' + classnames + ' ' + 'div .imgright').toggle()
			$('.' + classnames + ' ' + 'div .imgdown').toggle()
		})
	}
	sideclick('side_f')
	sideclick('side_y')
