	...
	var isLayui = window.layui && layui.define,
		...
		laydate = {
			...
			ready: function(fn) {
				var cssname = 'laydate',
					ver = '',
					path = (isLayui ? 'modules/laydate/' : '../css/') + 'laydate.css?v=' + laydate.v + ver;
	...
	*/
	...
	Class.prototype.config = {
		...
		showBottom: true //是否显示底部栏
			,
		btns: ['cancel', 'now', 'confirm'] //右下角显示的按钮，会按照数组顺序排列
			,
	...
	Class.prototype.init = function() {
		var that = this,
			options = that.config,
			dateType = 'yyyy|y|MM|M|dd|d|HH|H|mm|m',
		...
		lay.each(['min', 'max'], function(i, item) {
			...
			if(typeof options[item] === 'number') { //如果为数字
				...
				day < STAMP || (hms = [thisDate.getHours(), thisDate.getMinutes()]);
			} else {
				ymd = (options[item].match(/\d+-\d+-\d+/) || [''])[0].split('-');
				hms = (options[item].match(/\d+:\d+/) || [''])[0].split(':');
			}
			options[item] = {
	...
	Class.prototype.render = function() {
		...
		//  $('#layui-laydate1').css('background','red')
		//生成底部栏
		...
		lay(divFooter).html(function() {
			...
			lay.each(options.btns, function(i, item) {
				...
				if(isStatic && item === 'cancel') title = options.lang === 'cn' ? '重置' : 'Reset';
				btns.push('<span lay-type="' + item + '" class="laydate-btns-' + item + '">' + title + '</span>');
			});

			html.push('<div class="laydate-footer-btns clear">' + '<span class="setime" style="float:left;display:none">' + '时间间隔：' + '<input type="text" style="outline:none;width:80px;margin:0 5px 0 0;border:1px solid lightgray;ime-mode:disabled" id="timelv" onpaste="javascript:return false" onkeypress="myKeyPress(event)" onkeyup="mykeyUp(this)">' + '分钟' + '</span>' + '<span style="display:none;float:left" class="checkbox">' + '<input type="checkbox" style="margin-right:3px;width:15px;height:15px;vertical-align:middle">' + '自动模式开启' + '</span>' + '<span style="display:none;float:left" class="start">' + '<input type="checkbox" style="width:15px;height:15px;vertical-align:middle;margin-right:3px;">' + '开启' + '</span>' + btns.join('') + '</div>');
			return html.join('');
		}());
		//插入到主区域
	...
	$('.laydate-footer-btns span:last-child').css('height', '100px')
	//底部按钮
...
//					console.log(that.format[0] + that.format[1] + that.format[2])
//					console.log(that)
					that.done();
//					console.log(that.parse())
					that.setValue('<span class="tlv" style="display:none;">' + '定时检测心率 : ' +
						'</span>' + '<span class="tstep" style="display:none">' + '定时检测计步 : ' +
						'</span>' + '<span class="tpos" style="display:none">' + '定时定位 : ' +
						'</span>' + '<span class="tsleep" style="display:none">' + '睡眠监测设置 : ' +
						'</span>' + '<span class="sitset" style="display:none">' + '久坐提醒设置 : ' +
						'</span>' + '<span style="color:red;font-weight:bold">' + that.parse() +
						'</span>' + '<span class="tdistance" style="display:none">' + ',间隔 : ' +
						'</span>' + '<span style="color:red;font-weight:bold" onpaste="javascript:return false" onkeypress="myKeyPress(event)" onkeyup="mykeyUp(this)">' + document.getElementById('timelv').value +
						'</span>' + '<span class="tminu" style="display:none">' + '分钟' + '</span>').remove()
				}
			};
